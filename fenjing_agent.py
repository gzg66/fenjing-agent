...
⚠️ **首帧必须包含主角（关键！）**：由于视频通过「首帧图 → 视频」方式生成，首帧图是角色外观的**唯一视觉参考**。
  因此分镜脚本的**第一段时间轴**画面描述中**必须包含主角/核心角色**。
  即使是全景或远景镜头，主角也必须以可辨识的形态出现在画面中（例如：主角从远处飞来、主角站在画面一角俯瞰全景、主角的背影/剪影出现在前景等）。
  如果原始叙事中第一幕不包含主角，你必须调整叙事结构，将主角自然地融入第一幕。
...
9. **首帧角色一致性**：分镜脚本的第一段时间轴（即首帧画面）中**必须包含主角**。首帧图是视频生成的唯一视觉输入，若首帧无主角则后续画面的角色外观将无法保持一致
"""
